...
export const FileService = {
  async create(disciplineId, exerciseId, file) {
    ApiService.setAuthorizationHeader();
    try {
      const resp = await Vue.axios.post(
        `/disciplines/${disciplineId}/exercises/${exerciseId}/files/`,
        file,
  ...
  async deleteById(disciplineId, exerciseId, fileId) {
    ...
    try {
      const resp = await Vue.axios.delete(
        `/disciplines/${disciplineId}/exercises/${exerciseId}/files/${fileId}/`
      );
      let objResp = ApiService.getSuccessData(resp);
      return objResp.data;
    } catch (err) {
